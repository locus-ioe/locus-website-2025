      ...
      <div className='mx-auto flex max-w-7xl h-14 sm:h-16 md:h-18 lg:h-20 items-center justify-between rounded-full border border-zinc-800 bg-zinc-900/95 px-3 sm:px-4 md:px-6 backdrop-blur-sm transition-all duration-300 shadow-lg'>
        <Link to='/' className='flex items-center gap-2 z-[110]'>
          <div className='h-10 sm:h-12 md:h-14 lg:h-16 w-auto relative'>
            <img src='/assets/logodark.png' alt='Locus 2026 Logo' className='h-full w-auto object-contain' />
          </div>
        </Link>
        ...
        <nav className='hidden lg:flex items-center gap-1 z-[110]'>
          ...
          <div className='relative' ref={menuRef}>
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className='flex items-center gap-1 px-3 py-2 text-sm font-medium text-primary transition-colors rounded-lg'>
              More
            ...
            {isMenuOpen && (
              <div className='absolute right-0 mt-2 w-48 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl z-[120] overflow-hidden'>
                ...
                <a href='https://cit-worldlink.netlify.app/' target='_blank' rel='noopener noreferrer' className='block px-4 py-3 text-sm text-primary transition-colors' onClick={() => setIsMenuOpen(false)}>CIT-MAP</a>
              </div>
            )}
          </div>
        </nav>

        <button onClick={() => setIsMobileOpen(!isMobileOpen)} className='lg:hidden z-[110] p-2 rounded-lg transition-colors' aria-label='Toggle menu'>
